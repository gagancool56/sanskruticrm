...
class Saleordermodel extends CI_Model
{
    public $soid = '';
    ...
    function view()
    {
        ...
        if (!empty($soid)) {
            AddHierarchy('so');
            $res = $this->db->select('so.*,concat(\'SAF-\',soid)orderno,DATE_FORMAT(orderdate, "%d-%m-%Y") as orderdate,party_id2descr(partyid)descr_partyid')->where('soid', $soid)->where('orderstatus <>', 'CANCELLED')->get('so');
            ...
            if ($res->num_rows() > 0) {
                ...
                $res = $this->db->select('sodet.*,item_id2descr(itemid)descr_itemid')->where('soid', $soid)->get('sodet');
                if ($res->num_rows() > 0) {
                    foreach ($res->result() as $row) {
                        $filterdata = array();
                        foreach ($row as $key => $val) {
                            $filterdata['SODET[' . strtoupper($key) . '][]'] = $val;
                        }
                        $result[] = $filterdata;
                    }
                    // pr($result, 1);
                    add_rowdata("SODET", $result);
                }
            }
        }
    }
    ...
    function save()
    {
        ...
        if ($status === true) {
            echo json_encode(array('success' => true, 'message' => record_saved_message(), 'redirecturl' => show_record_redirect_url($this->soid, 'so')));
        } else {
    ...
    private function _save_order()
    {
        ...
        if ($insertdata['PAYMENTSTATUS'] == 1 && empty($insertdata['PAYMENTMETHOD'])) {
        ...
        if (!$this->db->insert('so', $insert)) {
            return $this->db->error();
        }
        $this->soid = $this->db->insert_id();
    ...
    function revise()
    {
        ...
        if ($status === true) {
            echo json_encode(array('success' => true, 'message' => record_revised_message(), 'redirecturl' => show_record_redirect_url($this->soid, 'so')));
        } else {
            echo json_encode(array('success' => false, 'message' => $status));
        }
    }
    ...
    {
        $this->db->trans_start();
        $this->soid = $this->input->post('CRM_SOID');
        ...
        $insert = $insert['data'];
        // pr($insert,1);
        if (!$this->db->where('soid', $this->soid)->update('so', $insert)) {
            return $this->db->error();
        }
        ...
        $EXISTINGSODETIDS = array_column($this->sodetrows(), 'sodetid');
